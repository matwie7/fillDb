...
public class Main {
    ...
    public static void doThings() {
        ...
        IntStream.range(0, 100).forEach(i -> {
            ...
            pesel = randomValuesProvider.getRandomPesel();
            dbInputData.add(new Klient(pesel, randomValuesProvider.getRandomName(), randomValuesProvider.getRandomSurname(), randomValuesProvider.getRandomAddress(), randomValuesProvider.getRandomPhoneNumber()));
            klienci.add(pesel);
        ...
        wycinki.stream().forEach(i -> {
            ...
            dbInputData.add(new Transakcja(i, klienci.get(random.nextInt(klienci.size())), String.valueOf(amount), String.valueOf(price), String.valueOf(amount * price)));
            dbInputData.add(new Zlecenie(i, pracownicy.get(random.nextInt(pracownicy.size()))));
            dbInputData.add(new Zlecenie(i, pracownicy.get(random.nextInt(pracownicy.size()))));
        });

//        fill_db.ScriptAsFileSaver.saveAsScript(dbInputData);
